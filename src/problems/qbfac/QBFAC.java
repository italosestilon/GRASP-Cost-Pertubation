...
/**
 * A quadractic binary function (QBFAC) is a function that can be expressed as the
 * sum of quadractic terms: f(x) = \sum{i,j}{a_{ij}*x_i*x_j}. In matricial form
 * a QBFAC can be expressed as f(x) = x'.A.x 
 * The problem of minimizing a QBFAC is NP-hard [1], even when no constraints
 * are considered.
 * 
 ...
 *
 */
public class QBFAC implements Evaluator<Integer> {
	...
	/**
	 * The matrix A of coefficients for the QBFAC f(x) = x'.A.x
	 */
	public Double[][] A;

	/**
	 * The constructor for QuadracticBinaryFunction class. The filename of the
	 * input for setting matrix of coefficients A of the QBFAC. The dimension of
	 * the array of variables x is returned from the {@link #readInput} method.
	 * 
	 * @param filename
	 *            Name of the file containing the input for setting the QBFAC.
	 * @throws IOException
	 *             Necessary for I/O operations.
	 */
	public QBFAC(String filename) throws IOException {
	 ...
	 * is required to perform the matrix multiplication which defines a QBFAC.
	 * 
	 * @param sol
	...
	/**
	 * {@inheritDoc} In the case of a QBFAC, the evaluation correspond to
	 ...
	 * {@link #evaluateInsertionQBFAC(int)}, {@link #evaluateRemovalQBFAC(int)} and
	 * {@link #evaluateExchangeQBFAC(int,int)}.
	 * 
	 * @return The evaluation of the QBFAC.
	 */
	@Override
	public Double evaluate(Solution<Integer> sol) {

		setVariables(sol);
		return sol.cost = evaluateQBFAC();

	}

	/**
	 * Evaluates a QBFAC by calculating the matrix multiplication that defines the
	 * QBFAC: f(x) = x'.A.x .
	 * 
	 * @return The value of the QBFAC.
	 */
	...
	public Double evaluateInsertionCost(Integer elem, Solution<Integer> sol) {
		...
		setVariables(sol);
		return evaluateInsertionQBFAC(elem);

	}

	/**
	 * Determines the contribution to the QBFAC objective function from the
	...
	public Double evaluateInsertionQBFAC(int i) {
		...
		return evaluateContributionQBFAC(i);
	}
	...
	public Double evaluateRemovalCost(Integer elem, Solution<Integer> sol) {
		...
		setVariables(sol);
		return evaluateRemovalQBFAC(elem);
	...
	public Double evaluateExchangeCost(Integer elemIn, Integer elemOut, Solution<Integer> sol) {
		...
		setVariables(sol);
		return evaluateExchangeQBFAC(elemIn, elemOut);
	...
	public Double evaluateExchangeQBFAC(int in, int out) {
		...
		if (in == out)
			return 0.0;
		if (variables[in] == 1)
			return evaluateRemovalQBFAC(out);
		if (variables[out] == 0)
			return evaluateInsertionQBFAC(in);

		sum += evaluateContributionQBFAC(in);
		sum -= evaluateContributionQBFAC(out);
	 ...
	 * solution. This method is different from {@link #evaluateInsertionQBFAC(int)},
	 ...
	 *         insertion.
	 */
	private Double evaluateContributionQBFAC(int i) {
	...
	/**
	 * Responsible for setting the QBFAC function parameters by reading the
	...
	/**
	 * A main method for testing the QBFAC class.
	 * 
	 * @param args
	 * @throws IOException
	 */
	public static void main(String[] args) throws IOException {
		...
		QBFAC qbfac = new QBFAC("matrix40");
		qbfac.printMatrix();
		...
		for (int i = 0; i < 10000; i++) {
			for (int j = 0; j < qbfac.size; j++) {
				if (Math.random() < 0.5)
					qbfac.variables[j] = 0.0;
				else
					qbfac.variables[j] = 1.0;
			}
			System.out.println("x = " + Arrays.toString(qbfac.variables));
			Double eval = qbfac.evaluateQBFAC();
		...
		System.out.println("x = " + Arrays.toString(qbfac.variables));
		System.out.println("f(x) = " + qbfac.evaluateQBFAC());
